...
import { BookData } from "../api/add/route";
...
function ArrayInput(props: {
    ...
    return (
        <div className="w-full grid sm:grid-cols-6 items-center">
            ...
            <input type="button" onClick={handle_submit} value="Lägg till" className="btn-primary" />
...
export default function AddForm(props: { reset: () => void }) {
    const [state, set_state] = useState<Partial<BookData>>({});

    function handle_submit(event: FormEvent) {
        event.preventDefault();

        if (!((state.authors && state.authors.length < 0) || (state.themes && state.themes.length))) {
            return;
        }
    ...
    return (
        ...
        >
            <Input type="text" id="title" label="Titel" required handle_change={handle_change} />
            <ArrayInput id="authors" label="Författare" required handle_change={handle_array_change} />
            <ArrayInput id="themes" label="Teman" required handle_change={handle_array_change} />
            <Input
                type="number"
                id="chapter_length"
                label="Kapitellängd"
            ...
            <ArrayInput id="abstract_school_words" label="Abstrakta skolord" handle_change={handle_array_change} />
